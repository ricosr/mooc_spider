...
def index():
    question = mongo.db.questions #链接包含内容的 mongodb
    if request.method == 'GET':
        return render_template('index.html')
    else:
        query = request.form.get('query') #从表单中获取用户的输入
        print(query)
        result = question.find_one({'content': query})  #从mongoDB中找到这个query
        print(result)
        if result:
            context = {
                'questions': question.find({'content': query}).sort("create_time", -1)
            }
